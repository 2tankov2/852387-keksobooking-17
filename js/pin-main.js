...
(function () {
  ...
  window.PINMAIN_WIDTH = 65;
  window.PINMAIN_HEIGHT = 87;
  ...
  var locationMap = {
    ...
    max: {
      x: window.map.widthMap - window.PINMAIN_WIDTH,
      y: 630
    }
  };
  ...
  window.pinMain.addEventListener('mousedown', function (evt) {
    ...
    var onMouseMove = function (moveEvt) {
      ...
      window.map.pinMainLocation = startCoords;

      window.pinMain.style.left = getPosition(window.pinMain.offsetLeft, shift.x, locationMap.min.x, locationMap.max.x);
      window.pinMain.style.top = getPosition(window.pinMain.offsetTop, shift.y, locationMap.min.y, locationMap.max.y);
    };
